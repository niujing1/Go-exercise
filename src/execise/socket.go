...
func read(conn net.Conn) (string, error) {
	readBytes := make([]byte, 1) //初始化为1，先判断一次是不是结束标记
...
func serverGo() {
	...
	printServerLog("Got listener for the server.(local address is %s.\n)", listener.Addr())
...
func handleConn(conn net.Conn) {
	...
	for{
		...
		printServerLog("Sent response (write %d bytes): %s", n, respMsg)
	...
	for j := 0; j < requestNumber; j++ {
		...
		if err != nil {
			if err == io.EOF {
				...
				printClientLog(id, "ReadError: %s\n", err)
			}
			break
		}
		printClientLog(id, "Receive response: %s\n", strResp)
	}
}
